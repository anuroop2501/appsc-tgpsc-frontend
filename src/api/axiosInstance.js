...
const api = axios.create({
  ...
  timeout: 120000, // 120s — AI calls (Claude + RAG) can take 30-60s on first request
})
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        setAuth({ token: newToken, refreshToken: data.refreshToken || refreshToken, user: useAuthStore.getState().user })
